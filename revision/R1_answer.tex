...
article, such pseudo-proxy explorations will be considered in future
work. 
...
\begin{enumerate}
\item Why use a previous reconstruction (PS04) as a reference to evaluate your reconstruction? The previous reconstruction unnecessarily represent the truth better. (This is not answered yet!)
\item Whether the scores are proper? (Luis argued they are proper)
\item State smaller scores mean better. (Easy and done.)
\item Consider psuedoproxy experiments to evaluate the reconstruction. (This we can add as future research.)
\end{enumerate}
}
