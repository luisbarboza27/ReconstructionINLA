...
version are shown in Table \ref{tab:screening}.\bl{the ``average correlation" or
  just the ``correlation"}\lb{average correlation}
\begin{table}
  \centering
...
\end{tabular}
  \caption{Comparison of $RP$'s obtained with and without screening procedure. \textbf{Sc}: Screened dataset, \textbf{Unsc}: Unscreened dataset, \textbf{CV}: coefficient of
    variation \bl{is this the var/mean or just the variance?}\lb{It is sd/mean}, \textbf{T}: observed temperature (1900-2000) }
  \label{tab:screening}
\end{table}
