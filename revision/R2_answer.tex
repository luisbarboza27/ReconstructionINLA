...
\bl{Yes, we assumed independent residuals. Barboza et al. (2014) found that the error structure is unimportant when forcings are included in the reconstruction, but now we also had reconstruction without forcings, so I am not sure how to address this question. Should we try the AR(1) model as well?}
...
\bl{It sounds complicated, but the reviewer pointed the function in INLA to use, so we may try the ``rw2" model?}
