...
this figure.
\bl{Luis, if there is no sampling involved, then INLA must calculate the
  posterior distribution directly. The calculation gives this bimodal? Would the
  bimodel appear because of the bins in the x-axis you choose to make the
  pdf?}\lb{Yes, the direct calculation gives bimodal. INLA gives the set of x-axis
  and y-axis values to plot or evaluate the density. I don't change those values
at all.}\bl{Can we add this to the end? ``We took the default values in x-axis when applying INLA to compute the posterior density. The small bimodal pattern also looks strange to us. We conjecture that the bimodal may be partly caused by the resolution along the x-axis when computing this posterior density. At this time, we cannot give a better answer on this, but we will continue to look into this issue." Does INLA allow you to take a quick check of whether you can change the resolution of x-axis and see if the results are different? }
