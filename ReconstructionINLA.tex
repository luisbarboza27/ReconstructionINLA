...
measures using the INLA's prediction intervals and the observed
  ...
  reconstruction\footnote{The smoothing version was computed with a Butterworth
    low-pass filter with a 50-year cutoff and order equal to 4.} and the borehole-based
...
% bit colder than the two best ones.
...
a single reduced proxy was aproximately 15 seconds \footnote{This comparison
...
\section{Discussion}
We have carried out a Bayesian inference of global mean surface temperature over
the past 2,000 years. By leveraging INLA to lighten the computational burden, our framework allows to investigate a wide range of choices for the model and data reduction strategy.  We validated the result using instrumental data over the 1850-1899 period, and using borehole temperature inversions prior from 1600-1899. The former constrains mostly variability on interannual scales. The latter constrains multi-decadal to centennial behavior.  

We have found that:

\begin{itemize}
    \item The data reduction techniques give roughly equivalent results, with
      sPCR and PCR performing marginally better in most cases.
    \item The model choice is highly consequential. Model "Mixed" is the most
      ...
      appears to perform at least as well as other choices. The existence of the
      additional term in the mean component of equation \eqref{eq:M3} allows us
      to model information that is not included into the external forcings,
      besides giving an additional dependence structure to the error term.
    \item In cases where both are computed, INLA allows to approximate the MCMC
      ...
      model available (1 reduced proxy), otherwise the computational time of the
      MCMC for more complex models like the ones presented in Figure
      \ref{fig:paleoCE1} was practically impossible to perform. 
    \item Temperature levels are unprecedented in the past 2,000 years, while
      the rates are only anomalous on scales longer than 50 years. There too,
      the twentieth century stands out as warming more rapidly than any other
...
It is important to add that one of the limitations of our analysis is the strong
...
certainty the testing error of the models, but for reasons of availability of
additional comparison information it can be considered a challenge for this type
of problem. Another important limitation is the modeling of spatial variability
in the reduced proxies. At least INLA has proved to be very computationally
efficient, so it could be considered in the future the extension of our models to the spatio-temporal case.
