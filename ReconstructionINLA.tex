...
{\bf the number of available observations} in the calibration period defined as
1900-2000CE \footnote{The choice of the calibration period is based on the
  arguments shown in \cite{Barboza2014}} (101 observations)\lb{what is the problem of this?} and (2) the number of proxies is very close to the
{\bf number of available observations}\lb{Are these two number of available observations the same thing?}, which can cause overfitting issues or
...
\item[Sparse Partial Least Squares (sPLS)] 
  Partial least squares seeks to reduce the dimension of the design matrix in the 
  linear regression models that suffer from high-dimensionality issues through a latent matrix
  whose components maximize the linear correlation \lb{with respect to the
  dependent variable together with its variance. Not sure what is the linear correlation with respect to...  } The sPLS method introduces sparseness to the partial least squares
  ...
  thresholding parameter\lb{suddenly there is a thresholding parameters, needs to be introduced eariler how this is used in the sPLS } of this method is found using a 10-fold cross-validation criteria.    
\item[Sliced Inverse Regression (SIR) with CSS selection method]
  ...
  (also kwown as \textit{dimension reduction space})\lb{If it is nonparametric, why the covariables has coefficients, sounds like a linear model?}.
  ...
  the conditional expectation of the design matrix given the response\lb{Solve an eigenvalue decomposition problem or directly do the eigenvalue decomposition? covariance matrix of the desiagn matrix? }. The
  estimation of the covariance matrix requires to partition the dependent variable into subgroups, called \textit{slices}.
  The SIR method can capture both linear and nonlinear \lb{linear or nonliear, or both linear and nonlinear} associations between the response and
  ...
  each slice \cite{Wu2010}. Because of this, we opt to incorporate the CSS \lb{spell out CSS and QZ} variable 
  selection procedure into the SIR-QZ method (see
  \cite{Coudret2014} and \cite{Coudret2017}). This compound method allows us to select
  reduced models that are close to the complete model (possibly with excess
  of dimension) in capturing the linear correlation among their SIR-QZ indices \lb{what are SIR-QZ indices? why need to get the linear correlation among indices?}. 
  We studied the association between proxies and temperatures
  through a linear regression among the chosen EDR directions \lb{what is EDR direction? what is a linear regression among directions?} using marginal
  dimension tests (see \cite{Cook2004}) and the anomalies observed
  during the calibration period\lb{Why suddenly talk about marginal test and the anomalies? Your description of a method is not smooth to follow. Often some terminology pops up without any earlier introduction}. 
...
\end{description}
Finally, we computed the reduced proxies by predicting\lb{compute the reduced proxies by prediction? should it be the reverse order?} the anomalies in the
