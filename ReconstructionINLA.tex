...
through the notion of Reduced Proxy ($RP$). First of all, we centered and
...
\item[Sparse Partial Least Squares (sPLS)] 
  ...
  design matrix in  
  linear regression models through a latent matrix whose columns maximize \bl{can we replace the rest of the sentence by "their linear correlation with the responses"}the 
  product of its linear correlation with respect to the response and its
...
\end{description}
The data reduction allows us to fit linear regression models between temperatures and proxies. After we fit a linear regression model under each of the five data reduction methods and for each of the eight proxy groups listed in Table \ref{tab:distdate}, we further reduce the proxies into a single reduced proxy series following \cite{Barboza2014}. All reduced proxies are
shown in Figure \ref{fig:RPs} in the Appendix. 
The reduced proxies obtained under different data reduction methods are highly correlated, except that the reduced proxies under PCR show a lower level of correlation with others, as we can see in Figure \ref{fig:CorrRPs} in the Appendix.  
%Pba
%Pba2
...
in such case the forcings will introduce circularity issue. We therefore will also explore the model that substitutes forcings by a smooth function defined as a linear combination of spline bases.
...
\item $B_t^{k,\tau}$: $k$-th B-Spline basis function at time $t$ with knot
  sequence $\tau$ (\cite{DeBoor2001} and \cite{Ramsay2005}). Here we choose cubic B-spline bases.  
\end{itemize}
We then define the first level of BHM as 
$$ RP_t^i&=\alpha_0^i+\alpha_1^iT_t+\epsilon^i_t, $$
where $\{\alpha^i_j\}$ random parameters for $i=1,\ldots,N$ and $j=0,1$,  $\epsilon^i_t$ are normally-distributed random variables with finite variances
$\{\sigma^2_{\epsilon^i}\}$.  \bl{Are you defining random effects model? I don't see this is the case. Define what is N here? Since you have 8 time periods, the RP should have 8 segments, you concatenate them into one single series?}

The second level will be defined in two different forms depending on whether forcings will be included. We have

{\bf Model NF}  $$T_t&=\beta_0+\sum_{k=1}^{K(\tau)}\beta_k B_t^{k,\tau}+\eta_t,$$
where $\beta_k$ are coefficients for B-spline bases, \bl{define $K(\tau)$}, and $\eta_t$ are also normally-distributed random variables with finite variances $\sigma^2_{\eta}$. For simplicity, the error terms $\epsilon^i_t$ and $\eta_t$ are assumed to be independent. The main idea of this model is to evaluate the central hypothesis of this article, that is, the paleoclimate reconstruction can perform  well without taking into account the external forcings, as long as a smoothing function has been included to approximate the dynamic behavior of the temperature series. In order to have a baseline model to evaluate Model NF, we define 

{\bf Model WF}
$$T_t&=\beta_0+\beta_1S_t+\beta_2\tilde V_t+\beta_3\tilde C_t+\eta_t.$$
Both first level and second level models are defined for
$t=1,\cdots,2000$ (Common Era). \bl{should $t$ also include the calibration period?} It is important to add that in Models NF and WF we simply assume
...
\bl{Suggest to use the above to replace the rest of this section}
We then define two types of models as extensions of \cite{Barboza2014}. %taking advantage of more reduced proxies:
\begin{description}
\item[State-Space model without forcings (Model SSnF)]\bl{Why it has the name ``state-space" model? This name is more for karman filters, right? Also is each model has to start with SS, then we should not include SS in the name as this is not helpful to differentiate different models.}
...
Denote the set of random \bl{again, why this is called random parameters?} parameters as
...
\end{align*}
\bl{I think we need to add the following to connect INLA to our problem, Otherwise it is purely description of INLA method. Please add description of $f(z_l)$} In our models, if we consider $y_i$ as the temperature anomaly,  $\eta_i$ the underlying mean temperature of interest, $x_m$ the reduced proxies, and $f(z_l)$ ????, then our models fall into the general specification of INLA. 
...
\end{align*}
\bl{Do we need this? I dont see we used this in the rest of the this subsection.-- and the marginal posterior distribution of each hyperparameter:
\begin{align*}
...
As Bayesian estimation, INLA requires priors for unknow parameters. Below is the list of priors we used in our models:
\begin{itemize}
