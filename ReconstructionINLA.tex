...
\label{sec:modelspec}
As we emphasized in the introduction, we use BHMs for our problem. The first level of BHM is always designated for modeling the likelihood of data. In the second level, forcings are often included to improve
...
proxy-based annual data allows to assume that a constant number of elements of
...
mean.\jeg{this assumption seems strange given that the number of proxies varies
  ...
  temperature can be
  modelled in an uniform way}. The choice of $K(\tau)=100$ for Model Mixed is
...
We now quantify the trade-offs of approximating the MCMC procedure using INLA.\jeg{that's what we're doing, right?}

The SSwF \jeg{change name} model in its simplest case (1 nest) was fitted in \cite{Barboza2014}
...
\section{Conclusions}
The impact of data reduction choices was explored by five popular dimension reduction techniques that allow us to compute reduced proxies under different temporal horizons and we were able to compare them through their
predictive capacity during a validation period. We also considered the model developed in \cite{Barboza2014}, and we explored to make it more complex by including more information from the proxy dataset, as well as by extending the reconstruction period by 1000 years. The participation of external forces in the hierarchical Bayesian model was tested by comparing its ability to describe the
...
approximating the adjustment with INLA, with which we were able to adjust Bayesian models with much
