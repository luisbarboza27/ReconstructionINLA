...
Earth's climate presents a continuum of variability, with periodic and non-periodic fluctuations ranging from 1 to $10^8$ years \cite{pelletier_power_1998,ghil2002natural,huybers_links_2006}. In particular, variability on scales of decades to centuries is of paramount importance for adaption and planning to anthropogenic climate change, yet is incompletely sampled by the relatively short historical record of wide-spread instrumental observations, going back to about 1850 CE \cite{AR5_chap5}. It is thus critical to reconstruct these variations from the paleoclimate record as quantitatively as possible.  A particular focus has been reconstructions of global or hemispheric temperature from high-resolution proxy observations \cite{Jones_Holocene09}.
...
Many methods have been developed to reconstruct past climates, particularly temperature:  principal component regression
\cite{MBH98,MBH99,luterbacher2004european,Wahl2012,Neukom:2014}, regularized forms of the expectation-maximization algorithm \cite{Schneider2001,bradley2005proxy,mann2005testing,mann2007robust,rutherford2003climate,steig2009,JEG10a,JEG10b,Guillot_AOAS2015}, canonical
correlation analysis \cite{smerdon2010pseudoproxy,Wang_CP2014,Evans_grl2014}, pairwise comparison
\cite{Hanhijarvi2013,Gergis2016}, data assimilation \cite{Lee_CD08,Hakim2016}, and Bayesian hierarchical models (BHMs). 

BHMs models offer several distinct advantages for paleoclimatic reconstruction. They can ($i$) treat different sources of uncertainty in a natural way ($ii$) incorporate prior knowledge of the system in a logically-coherent manner \cite{Jaynes04} (c) allow for both inference and prediction. 

Many studies have been conducted BHMs: \cite{boli1, tingley1,tingley2,tingley2013_Ext,werner2012pseudoproxy,Barboza2014} among others. Some of these studies have used space-state
...
Recently a method was proposed that jointly models the variability of the temperature series (as a latent process) as well as the variability of those climatic and biological variables that serve as approximations of this process \cite{Barboza2014}.  The authors found that ... \jeg{Luis, summarize your findings here}. The scope of the study was restricted by the computational requirements of the Monte-Carlo Markov Chain (MCMC) procedure, which limited the sensitivity analysis and forced severe levels of data reduction. 

In this article we leverage Integrated Nested Laplace
Approximations (INLA) to extend the work of \cite{Barboza2014}. By lightening the computational burden, INLA allows to (a) explore new model designs inspired by the physics of the problem; (b) consider various choices for data reduction; and (c) take the non-stationary nature of the observational network into account.  In addition, this work makes use the most up-to-date estimates of radiative forcing, as well as a state-of-the-art, open-access compilation of community-curated paleoclimate observations \cite{PAGES2kSD2017allauthors}. This ensures that our calculations are using the best available data and are completely
reproducible. %They can also be readily updated as the dataset changes over time. 
...
ice, lake and marine sediment, sclerosponge, speleothem and tree-ring archives \cite{Jones_Holocene09}.
...
past 2,000 years'' \cite{PAGES2KConsortium2013,Kaufman2014,PAGES2kSD2017allauthors}. The most recent effort \cite{PAGES2kSD2017allauthors} made all records available in a standard format (LiPD, \cite{lipd_cp}) readable in R, Python and Matlab \cite{lipd-util}, to ensure reproducible workflows.   
...
their closest spatial temperature record in the HadCRUT4.2 dataset \cite{Morice2012}. More
details on this ``screening'' procedure, which controls for the multiple test problem \cite{BenjaminiHochberg95}, can be found in \cite{PAGES2kSD2017allauthors}; it whittles down the database to 257 proxies (Fig.~\ref{fig:proxy}).   
\begin{figure}
  \centering
...
\jeg{Luis, are you using the GMST provided by HadCRUT4, or do you calculate your own from the spatial data?}
...
It is well-known that climate responds to external forcings, including natural forcings. In this work we take advantage of this knowledge for statistical modeling, and use the most recent compilations from the PMIP4-CMIP6 project \cite{JungclausGMD17,Kageyama_GMD2017}. The data consist of:
\begin{itemize}
...
\item The solar forcing data is computed from the SATIRE-H
  (Holocene) dataset \cite{Vieira2011}. Irradiance from SATIRE-H (Holocene) is
...
\item Greenhouse-Gases concentrations: hemispheric means of mole
  ...
  resolution \cite{Meinshausen_GMD2017} as well as ice core measurements prior to that date \cite{JungclausGMD17} (Fig.~\ref{fig:forcings}, bottom).
...
\section{Results.}
\label{sec:results}
\jeg{Here the baseline seems to be WF. WOuld it make sense to choose "Mixed" or "NF" instead?} 
...
validation interval \jeg{with smoothing}. We compared in this case Model WF with a single RP
...
the PCR method shows an improvement in terms of measures. \jeg{I bet my house that this will change when you lowpass filter temperature}
...
base in Model NF is selected according to the adjusted $R^2$ of a linear regression model between observed anomalies and
...
throughout all the reconstruction period. Note that the high prevalence of
annually-resolved proxy data allows to assume that a constant number of elements of
the BSpline basis is adequate to describe the component of the temperature
...
techniques of section \ref{sec:rp}. These results are
also shown in Table \ref{tab:comparisontot}. Note that the SPLS method achieves a better performance in terms of all the
...
WF model with the first reduced proxy from the sPCR method (the best choice
...
\ref{fig:paleo19004}, together with the results of the best
...
\jeg{can we use the Mixed model as baseline here?. You could put the figures for NF and WF in the Appendix, if need be.}
...
\section{Discussion}
We have carried out a Bayesian inference of global mean surface temperature over the past 2,000. By leveraging INLA to lighten the computational burden, our framework allows to investigate a wide range of choices for the model and data reduction strategy.  We validated the result using instrumnetal data over the 1850-1899 period, and using borehole temperature inversions prior from 1500-2000. The former constrains mostly variability on interannual scales. The latter constrains multi-decadal to centennial behavior.  

We have found that .. \jeg{summarize what the results actually tell us. To me these are:}

\begin{itemize}
    \item the data reduction techniques give roughly equivalent results, with PCR performing marginally better in most cases.
    \item the model choice is highly consequential. Model "mixed" is the most physically justifiable and appears to perform at least as well as other choices. 
    \ item In cases where both are computed, INLA allows to approximate the MCMC solution at a fraction of the computational cost, but with wider uncertainties
    \item Temperature levels are unprecedented in the past 2,000 years, while the rates are only anomalous on scales longer than 50 years. There too, the twentieth century stands out as warming more rapidly than any other century in the past 2,000 years. 
    \item All models that include forcings (WF, Mixed) show that carbon dioxide is the leading contributor to this warming \jeg{verify that this is true...}. 
\end{itemize}

%The impact of data reduction choices was explored by five popular dimension reduction techniques that allow us to compute reduced proxies under different temporal horizons and we were able to compare them through their predictive capacity during a validation period. We also considered the model developed in \cite{Barboza2014}, and we explored to make it more complex by including more information from the proxy dataset, as well as by extending the reconstruction period by 1000 years. The participation of external forces in the hierarchical Bayesian model was tested by comparing its ability to describe the average behavior of temperature anomalies with respect to a linear combination of BSplines. Of course, this adds more complexity to the hierarchical model and  it becomes more necessary to solve the slow behavior of the adjustment process when we employ MCMC techniques. This second challenge was successfully solved by approximating the adjustment with INLA, with which we were able to adjust Bayesian models with much greater complexity than \cite{Barboza2014} in a much shorter period of time.  Discuss about using INLA for space-time reconstruction...
