...
\begin{table}
  ...
  \label{tab:distdate}
\end{table}
An important aspect that we can notice from the distribution of Table
\ref{tab:distdate} is that the number of proxies is greater or at least very
...
period. Many different data reduction methods have been developed. We will focus on five popular ones below:  
...
\item[Sparse Partial Least Squares (sPLS)] 
  Partial least squares seeks to reduce the high-dimensionality issues of the
  design matrix in  
  linear regression models through a latent matrix whose columns maximize the
  product of its linear correlation with respect to the response and its
  variance. The sPLS method introduces sparseness to the partial least squares
...
\item[Sliced Inverse Regression (SIR) with CSS selection method]
  ...
  also known as \textit{effective dimension reduction} (EDR) subspace. \bl{If it is nonparametric,
    why the covariables has coefficients, sounds like a linear model?}\lb{It is
    nonparametric because the main model has the form
    $y=f(\beta_1x,\beta_2x,\ldots,\beta_kx,\epsilon)$ where $f$ is unknown, but the
  vector $x$ is p-dimensional (p covariates) and the $\beta$'s are parameters.}
  This subspace is obtained by solving an eigenvalue decomposition problem that
  ...
  response\bl{Solve an eigenvalue decomposition problem or directly do the
    eigenvalue decomposition?}\lb{The eigenvalue decomposition can't be
    performed directly, so it is solved in an approximate way}. The
  ...
  selection procedure into the SIR method and, in case of a larger number of
  covariates than observations, we employ the SIR-QZ method \footnote{The SIR-QZ
    method is based on the generalized Shur decomposition and it aims to solve
    the eigenvalue problem of SIR for undetermined cases.} instead  (see
  \cite{Coudret2014} and \cite{Coudret2017}).
  ...
  In our case, we studied the association between proxies and temperatures
  through a linear regression among a certain number \footnote{The number of EDR
  directions is determined by means of marginal
  dimension tests (see \cite{Cook2004}). } of EDR directions (orthogonal basis
  of the EDR subspace) and the anomalies observed
  during the calibration period. 
\item[Principal Component Regression (PCR)]
We fitted a regression model between the temperature and PCs of covariates in the calibration period. We selected the number of PCs in each of
the eight regressions as the minimum number that attains, for the first time, an adjusted
$R^2$ of at least 70\% in each case. The PCR is often used when the covariates are
highly-correlated or when the number of covariates is larger than the number of 
 observations. A caveat of this method is that principal components
with smaller contribution to variance are not necessarily the ones that
associate less with the dependent variable in a linear regression model (see
...
cross-validation. Comparine to PCR, the sPCR ensures the dimensionality
...
sPLS method (see \cite{Chung2013}).  
\end{description}
Finally, we computed the reduced proxies by predicting\bl{compute the reduced
  proxies by prediction? should it be the reverse order?}\lb{No, the reduced
  proxies are the predictions of the linear model between anomalies
  and the proxies after the application of the reduction methods, as we did in
  AOAS, 14.} the anomalies in the
...
shown in Figure \ref{fig:RPs} in the Appendix. Also note that
the obtained reduced proxies from different methods are highly correlated, except that the reduced proxies from PCR
method shows a lower level of correlation to those from the other methods,
...
\label{sec:modelspec}
As we emphasized in the introduction, Bayesian hierarchical models represent an interesting modeling option for this class of problems.
%The most popular model is to integrate all information using Bayesian
%hierarchical models.
We will adopt this modeling framework for this research as
well. In the second hierarchy of the BHM, forcings are often included to improve
...
the forcings will introduce circularity issue. We explore the model that
replaces forcings by a smooth function defined as a linear combination of spline basis.
